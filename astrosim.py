...
class AsteroidTracker:
    def __init__(self):
        ...
        self.ASTEROID_SPEED = 356  # Speed to reach Moon in 28 days (238900 miles / 672 hours)
    ...
    def calculate_trajectory(self):
        for hour in range(0, self.TOTAL_HOURS + 1):
            ...
            if dist_to_moon < 10000 and hour % 24 == 0:
                print(f"Day {days}: Distance to moon = {dist_to_moon:.2f} miles, Position: ({x:.2f}, {y:.2f}, {z:.2f})")
            
            if dist_to_moon < self.MOON_RADIUS and not self.collision_detected:
                self.collision_detected = True
                self.collision_time = days
                print(f"COLLISION! Distance: {dist_to_moon:.2f}, Day: {days}, Position: ({x:.2f}, {y:.2f}, {z:.2f})")
